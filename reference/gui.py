...
class SIZE:
    ...
    # 消息区域高度
    TEXT_BROWSER_HEIGHT = WORLD_HEIGHT - TECHNOLOGY_HEIGHT - PANEL_HEIGHT - DX - DX - DX - DX
    # 消息区行高
    TEXT_LINE_HEIGHT = TEXT_BROWSER_HEIGHT // NUMBER.TEXT_LINE

    # 窗口宽度
    WINDOW_WIDTH = DX + WORLD_WIDTH + DX + DX + ZONING_WIDTH + DX + DX + PANEL_WIDTH + DX
    # 窗口高度
...
class POSITION:
    ...
    # 区划左边坐标
    ZONING_LEFT = WORLD_RIGHT + SIZE.DX + SIZE.DX
    # 区划右边坐标
    ...
    # 面板左边坐标
    PANEL_LEFT = ZONING_RIGHT + SIZE.DX + SIZE.DX
    # 面板右边坐标
    PANEL_RIGHT = PANEL_LEFT + SIZE.PANEL_WIDTH

    # 科技板顶部坐标
    TECHNOLOGY_TOP = PANEL_TOP + SIZE.PANEL_HEIGHT + SIZE.DX + SIZE.DX
    # 科技板底部坐标
    ...
    # 消息区顶部坐标
    TEXT_BROWSER_TOP = TECHNOLOGY_BOTTOM + SIZE.DX + SIZE.DX
    # 消息区底部坐标
